...
app.post('/generate-thesis', async (req, res) => {
  ...
  const systemPrompt = `You are Kyle Mallien's senior acquisition strategist. Kyle is an INC 5000 entrepreneur and business acquisition mentor. His methodology: F.U.E.L. (Find, Underwrite, Elevate, Legacy). Buy box: service-based, recession-proof, 10+ years operating, 10+ employees, $1M–$5M revenue, 20%+ margins, SBA 7(a) financing, retiring founder (ages 58–70).

Return ONLY a raw JSON object — no markdown, no backticks, no preamble. The JSON must have EXACTLY these keys:

{
  ...
  "thesis_overview": "<p>3 rich HTML paragraphs personalized to this person...</p>",
  "unfair_advantages": [
    {"title": "Advantage Name", "body": "Specific explanation tied to their background..."},
    {"title": "Advantage Name", "body": "..."},
    {"title": "Advantage Name", "body": "..."},
    {"title": "Advantage Name", "body": "..."}
  ],
  "target_verticals": [
    {"rank": 1, "vertical": "Managed IT / MSP", "margin": "25–35%", "why": "Specific reason tied to their background..."},
    {"rank": 2, "vertical": "...", "margin": "...", "why": "..."},
    {"rank": 3, "vertical": "...", "margin": "...", "why": "..."},
    {"rank": 4, "vertical": "...", "margin": "...", "why": "..."},
    {"rank": 5, "vertical": "...", "margin": "...", "why": "..."}
  ],
  "roadmap": {
    "days_1_30": {
      "objective": "One sentence strategic objective for FIND phase, personalized to their edge...",
      "actions": [
        {"days": "Days 1–3", "action": "Specific action tied to their background..."},
        {"days": "Days 3–7", "action": "..."},
        {"days": "Days 7–12", "action": "..."},
        {"days": "Days 12–18", "action": "..."},
        {"days": "Days 18–25", "action": "..."},
        {"days": "Days 25–30", "action": "..."}
      ]
    },
    "days_31_70": {
      "objective": "One sentence strategic objective for UNDERWRITE phase...",
      "actions": [
    ...
    "days_71_100": {
      "objective": "One sentence strategic objective for ELEVATE & LEGACY phase...",
      "actions": [
  ...
  "scripts": {
    "ceo_letter": {
      ...
      "subject": "",
      "body": "Write a full personalized letter from [their first name] to a prospective seller. Warm, peer-to-peer tone referencing their industry/edge. No brackets except [Owner First Name] and [Business Name]."
    },
    "email_followup": {
      "label": "Email Follow-Up (Day 7 After Letter)",
      "subject": "Following up on my letter — [Business Name]",
      "body": "Short follow-up email referencing the physical letter. Warm, no pressure. Signed with their full name."
    },
    "discovery_call": {
      "label": "Discovery Call Framework — 20 Minutes",
      "subject": "",
      "body": "OPENER (2 min): [script]\\n\\nLEARN (8 min): [questions]\\n\\nEDUCATE (6 min): [script]\\n\\nCLOSE (4 min): [script]. Personalize to their background and target verticals."
    },
    "seller_intro": {
      "label": "Seller Introduction to Customers — Joint Letter",
      "subject": "An important message about [Business Name]",
      "body": "Joint letter from [seller name] and [their first name] to existing customers. Warm handoff. Reassure on team, service, relationships. Reference their specific background as a credential."
    }
  },
  "deal_structure": [
  ...
  "value_creation_levers": [
    {"title": "Lever Title", "body": "Specific lever tied to their background..."},
    {"title": "Lever Title", "body": "..."},
    {"title": "Lever Title", "body": "..."},
    {"title": "Lever Title", "body": "..."}
  ],
  "sba_financing": [
  ...
  "milestones": [
    {"day": "Day 30", "milestone": "Specific milestone personalized to their situation..."},
  ...
  "next_steps": [
    {"timeframe": "This Week", "action": "Specific action tied to their profile..."},
  ...
  "closing_insight": "One powerful closing paragraph personalized to their motivation and legacy goal."
}

Be hyper-specific. Use their name throughout. Every action item, script, and lever must feel written exclusively for them. The scripts must be complete and immediately usable.`;

  const userPrompt = `Generate the complete acquisition thesis JSON for this Elite Wealth Club attendee.
...
Make every section feel written exclusively for ${profile.name.split(' ')[0]}. The scripts must include their name and reference their specific background. Return only raw JSON.`;
  ...
  try {
    ...
    try {
      ...
      console.error('JSON parse failed, attempting repair. Length:', clean.length);
      ...
      try {
        ...
        return res.status(500).json({ error: 'Response was too long. Please try again.' });
      }
    }
...
tr:last-child td{background:#f5edd8;font-weight:700}
...
<h2>Your Unfair Advantages</h2>${(t.unfair_advantages||[]).map(a=>`<div class="advantage"><div class="advantage-title">${a.title}</div><div>${a.body}</div></div>`).join('')}
...
${['days_1_30','days_31_70','days_71_100'].map((k,i)=>{const ph=t.roadmap?.[k];const labels=['F — FIND (Days 1–30)','U — UNDERWRITE (Days 31–70)','E+L — ELEVATE & LEGACY (Days 71–100)'];return ph?`<div class="phase"><div class="phase-title">${labels[i]}</div><p><strong>Objective:</strong> ${ph.objective}</p><table><thead><tr><th>Days</th><th>Action</th></tr></thead><tbody>${(ph.actions||[]).map(a=>`<tr><td style="white-space:nowrap;width:120px">${a.days}</td><td>${a.action}</td></tr>`).join('')}</tbody></table></div>`:''}).join('')}
...
<h2>Deal Structure Framework</h2>
...
<h2>Post-Acquisition Value Creation</h2>${(t.value_creation_levers||[]).map(l=>`<div class="advantage"><div class="advantage-title">${l.title}</div><div>${l.body}</div></div>`).join('')}
<h2>SBA 7(a) Financing Path</h2>
